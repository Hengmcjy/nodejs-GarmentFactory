...
@Component({
    selector: 'app-s-rep-outs-overall',
    templateUrl: './s-rep-outs-overall.component.html',
    styleUrls: ['./s-rep-outs-overall.component.scss'],
    providers: [DialogService, MessageService],
})
export class SRepOutsOverallComponent implements OnInit, OnDestroy {
    formActive = 'repOutSourceOverall';
    ...
    ngOnInit(): void {
        ...
        this.lastColor = '';
        this.getRepCompanyOrderOutsource('dt');
    }

    getOutsourcefactoryIDQTY(facOutsID: string, orderID: string): string {
        //
        const orderProductFacOutQTYF =
            this.orderProductFacOutQTY.filter(i=>i.outsourcefactoryID==facOutsID
                && i.orderID==orderID);
    ...
    getOutsourcefactoryIDRemainQTY(facOutsID: string, orderID: string): string {
        ...
        const orderProductFacOutRemainQTYF =
            this.orderProductFacOutRemainQTY.filter(i=>i.outsourcefactoryID==facOutsID
                && i.orderID==orderID);
    ...
    // ## type = 'dt' , 'refresh'
    // ## dt = data temp
    async getRepCompanyOrderOutsource(type: string) {
        // console.log(type);
        ...
        this.repService.getRepCompanyOrderOutsource(this.company.companyID, ordertatus, seasonYear, type);
    ...
    getOrdersByOrderIDs(orderIDs: string[]) {
        ...
        this.ordersByOrderIDsSub = this.orderService.getOrdersByOrderIDsListener().subscribe((data) => {
            ...
            // ## grouping style
            this.currentCompanyOrder.sort((a,b)=>{ return a.orderID >b.orderID?1:a.orderID <b.orderID?-1:0 });
            // console.log(this.currentCompanyOrder);
    ...
    orderStyleColorSizeFilterF(outsFactoryID: string, orderID: string) {
        ...
        // console.log(this.orders);
        let colors: ColorS[] = [];
        const colorsF = this.orders.filter(i=>i.orderID == orderID);
        if (colorsF.length > 0) {
    ...
    findOrderQty(orderID: string) {
        const currentOrderStyleF = this.currentOrderStyle.filter(i => i.orderID === orderID);
    ...
    inputUserPassPopup() {
        const ref = this.dialogService.open(SmdConfirmImportantTaskComponent, {
            data: {
                id: 'staffRefreshoutsourceOverall',
                mode: 'RefreshoutsourceOverall',
            },
            header: 'Confirmation for Refresh outsource Overall',
            width: '30%'
        });
        ...
        ref.onClose.subscribe((data: any) => {
            ...
            if (data) {
                if (data.mode && data.mode === 'RefreshoutsourceOverall' && data.success) {
                    // console.log(data);
