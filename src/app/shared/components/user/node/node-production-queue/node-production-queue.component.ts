...
import { OrderProductQtyByOrderIDProductIDRep, QueueInfoRep, RepDataFormat1 } from 'src/app/models/report.model';
...
@Component({
    selector: 'app-node-production-queue',
    templateUrl: './node-production-queue.component.html',
    styleUrls: ['./node-production-queue.component.scss'],
})
export class NodeProductionQueueComponent implements OnInit, OnDestroy {
    pageActive = 'production-queue';
    ...
    // factories = this.userService.factories;
    stationID = '';
    nodeFlows: NodeFlow[] = [];
    ...
    private repCurrentProductQueueCFSub: Subscription = new Subscription;
    ...
    ngOnInit(): void {
        ...
        // console.log(this.nsService.orders, this.nsService.products);
        // console.log(this.factories);
        // console.log(this.factory);

    }

    getRepCurrentProductQueue(page: number) {
        let productIDs: string[] = [];
        this.orderProductQtyByOrderIDProductIDRep.forEach( (item, index) => {
            productIDs.push(item.productID);
        });

        // getRepCurrentProductQueue(companyID: string, factoryID: string, page: number, limit: number)
        this.repService.getRepCurrentProductQueue(this.company.companyID, this.factory.factoryID, productIDs, page, this.limit);
        if (this.repCurrentProductQueueCFSub) { this.repCurrentProductQueueCFSub.unsubscribe(); }
        this.repCurrentProductQueueCFSub = this.repService.getRepCurrentProductQueueCFUpdatedListener().subscribe((data) => {
            // console.log(data);
            this.queueInfoRep = data.queueInfoRep;
    ...
    paginate(event: any) {
        ...
        this.getRepCurrentProductQueue(+event.page + 1);
    ...
    showDataReport(repDataFormat1: RepDataFormat1, repListNameArr: string[]) {
        ...
        if (repListNameArr.includes('getRepCFNCurrentProductQtyByOrderIDProductID')) {
            ...
            this.getRepCurrentProductQueue(1);
